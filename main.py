...
# импорт библиотек
def logging(
        func):  # декоратор для логирования, принимает в аргумент саму функцию
    time_log_dt = datetime.now(
    )  # вызов функции настоящего времени сейчас в формате гггг-мм-дд чч:мм:сс.мс
    time_log_str = str(time_log_dt)  # преобразование неявного типа
    print("[SYSTEM_LOG] function start in " +
          time_log_str)  # конкатенация строк

    def wrapped():  # наследование аргументов декорируемой функции
        start_time = time.perf_counter(
        )  # perf counter - время начала выполнения
        res = func()  # само выполнение с наследованием аргументов
        print(time.perf_counter() -
              start_time)  # разница, длительность выполнения функции
...
class Neuron:  # суперкласс для Net
    ...
    def sigmoid(self) -> float:  # стандартное поведение
        return 1 / (1 + math.exp(-self.num)
                    )  # вызов функции библиотеки math с аргументом -num
...
class Net(Neuron):  # специализированных подкласс, наследование от Neuron
    ...
    def set_type_nn(
            self,
            type_net):  # присвоение локальной переменной внешнего значения
...
def arch_oop() -> None:
    ...
    perceptron1.set_type_nn(
        'perceptron')  # установка значения для типа нейронки
